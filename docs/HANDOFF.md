...
15. **多版本接入 + np8 提速 + 孤儿进程 bug（2026-08-30 ~ 09-06）**：`_newest_session` 升级多根扫描（`DSH_SESSION_ROOTS` 分号分隔扩展根），本机 rc1 / `.rc8-lab-home` / `home-coexist` 三根自动跟随活跃会话——多 DSH 版本共用一个观察器、一个库。1.8B 并发 `-np 4→8`、`-c 1024→2048`（实测显存约 3GB）。**孤儿进程 bug（严重，跨九天）**：08-28 的旧参数进程（np=1）一直占用 8199，`ensure_model` 的 health 复用只查存活不比对参数 → 期间所有参数修改静默不生效，09-06 排查"翻译慢"才发现全程单路串行九天；手动清孤儿后 np=8 生效。〔**待办**：`ensure_model` 复用前须校验进程 cmdline 的 -np/-c 与目标一致，不一致则收养重启；`start_watcher.cmd` 的 UTF-8 中文路径在 cmd 默认 GBK 代码页下会乱码，须改 `chcp 65001` + 无 BOM 存储。〕同窗完成：四版本 AGENTS.md 注入本机翻译服务说明（rc1 追加，rc8-lab / home-coexist / 默认根新建——同机 agent 发现性）；`app\AGENT-USAGE.md` 落地主动调用文档。库增长：75,353 → 82,000+ 句（实时句持续入库，7B 挖矿循环常开自动校正）。

16. **隔壁环境救援（2026-09-06 上午）**：官方桌面版 DeepSeek Harness（用户数据 `AppData\Roaming\deepseek-harness-desktop\harness-home`）装 dsh-think-translate 后启动失败，报 `duplicate loader entry id` 三连（ui-task-board → live-stats 逐个冒头）。真凶：web profile 的 bundles 同时声明 linxin 全家桶 `@linxin666/dsh-web-ui-all`（内嵌 task-board / live-stats / ssh / describe-image 四件套）与其中三个单件——loader 撞到第一个重复即拒载整树；08-17 同环境炸过 `describe-image`，同一结构性问题。修复：程序化去重（递归收集全家桶内嵌包名 ∩ bundles 单件 → 全部移除，零功能损失）。**次生雷（自伤，诚实记录）**：PowerShell `Set-Content -Encoding UTF8` 写 package.json 带 BOM → Node `JSON.parse` 拒收 → `readProfileManifest` 崩，靠改前备份 `package.json.bak-*` 重建恢复。教训三条：a) 装插件先查全家桶是否已含同件；b) profile 的 JSON 一律 python `json.dump`（无 BOM）写，禁用 PS5.1 的 UTF8；c) 改配置前先备份——本次救场两次。think-translate 1.0.12 已在隔壁 web 侧正常（unclk 活跃迭代至 1.0.12，出厂 config 默认指向本地端点 18765）。

17. **模型死亡自愈缺失 + 假阳性验证（2026-09-06 深夜，隔壁发现）**：隔壁实测发现 llama-server 中途死亡后无任何自愈、塌约两小时无人发现。根因：`ensure_model` 在进程死亡后 `_mproc[0]` 仍持旧 Process 对象（非 None），重拉条件 `is None` 永不成立 → 永不重生；且 `live["model_up"]` 只在成功时置 True、失败不置 False → **观察页/状态接口说谎**。修复三连：a) `ensure_model` 重写——端口不通即落 `model_up=False` 并无条件重生（90s 加载宽限 + 模型锁防并发误杀 + Popen 异常留痕）；b) 新增 `model_guard_loop`（60s 周期探活线程，异常不静默）——与隔壁守护任务（管 watcher 进程）形成双层保险：守护管进程、guard 管模型；c) `decoder` 降噪改为 30s 时间节流（首版"连续计数"被逐帧成功清零，节流失效——教训：交替型事件的降噪必须按时间窗）。**假阳性教训（双方各犯一次）**：隔壁称"model_up=True、翻译 292ms 正常"实则 8199 已拒连——model_up 是缓存字段非实时探测，命中句走库正常掩盖了实时层瘫痪；验证模型存活必须直探 `<MPORT>/health`。**pythonw → python（session 0）→ 回到 pythonw（终案）**：pythonw（GUI 子系统）下曾出现 llama 未重生，一度归罪于 pythonw 改用 python.exe——但 python.exe 经 `cmd /c` 前台等待反而带回常驻控制台（cmd 等 console 子进程）；深查后 pythonw 疑案翻案（上次失败系启动时序偶发，非 pythonw 之罪——PP-OCR 的 pythonw 服务长期健在可为旁证）。终案配置：`start_watcher.cmd` 用 pythonw（真无窗，cmd 壳闪退 <1s）+ 日志落 `app\watcher.log`。实测：杀模型 → guard 70s 内重生 np=8 → 8199/18765 双 health ok → 可见窗口 0 → 且多根会话切换自愈同轮实锤（watcher 自动跟随最活跃会话文件）。

18. **发布刷新 v1.0.0（2026-09-07）**：重打包替换 Release asset，含全部近期修复（自愈/无窗/多根/降噪/真实场景排查）。要点：a) 种子库从现役主库刷新——**export_clean.py 原 SRC 指 E: 侧旧缓存已过期**（差点导出旧库），修正为 D:\think-zh\app\cache\tm.sqlite3，86,321 → 剔 204 敏感 → 86,117 条，复扫零残留；b) 包白名单 17→19 项（+AGENT-USAGE.md、+start_watcher.cmd），start_watcher.cmd 用**通用模板 override**（相对路径 %~dp0、零本机路径）——仓库版含本机三根不入包，运行时与发布物分离；c) 新增 `out/audit_pack.py` 五维发布前审计（敏感模式/本机泄漏/功能完整性抽查/包-git 一致性/mining.off 语义）——**首跑即抓到真问题**：app\out\mining.off（挖矿默认停用标记）在 D: 侧丢失，恢复后入包；API_KEY 疑点为教学占位符误报（行级豁免 sk-xxx）；d) INSTALL 三修：第 5 步改引用包内无窗脚本、失败处理 np 4→8 内部不一致修复、新增显卡耗时预期区间。审计全绿后才上传（用户要求先审计后上传——流程固定化：以后每次发版必跑 audit_pack.py）。